...
{
    //Classes variables
    TutorialMasterScript tm;                                                //variable which is going to be used to simplify the code
    TutorialMasterScript.frameClass frameData;                              //variable which has access to the frame data
    TutorialMasterScript.tutorialClass tutorialData;                        //variable which has access to the tutorial  data (frame data included)

    //Editor Variables
    Vector3 tutorialsListVector = Vector3.zero;                             //used for scrolling of the tutorials list
    Vector3 frameClassListVector = Vector3.zero;                            //used for scrolling of the frames list
    Vector3 exceptionsListVector = Vector3.zero;                            //used for scrolling of the exceptions list

    bool showBaseSettings = true;
    ...
    GUIStyle infoText = new GUIStyle();                                     //text which shows brief info on tutorials and frames (e.g ID)
    GUIStyle boldText = new GUIStyle();                                     //text which acts like a header in the custom inspector
    GUIStyle errorText = new GUIStyle();                                    //text that displays any related errors tutorial/frame may have (e.g a missing GameObject)
    GUIStyle customTextField;                                               //custom style for a multiline text field
    
    GUISkin tmSkin;                                                         //declares a custom Tutorial Master skin
	...
	void MarkEditorDirty() 
	{
        if (!Application.isPlaying)                                                     //ensure that Unity isn't in play mode
        {
            ...
            #else                                                                       
		    EditorApplication.MarkSceneDirty();                                         //otherwise run this bit of code
            #endif
        }
	}
